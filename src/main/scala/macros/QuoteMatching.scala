...
object QuoteMatching {
  inline def pmOptions(inline opt: Option[Int]) = ${ pmOptionsImpl('opt) }
  ...
  // List(1,2,3).map[String](_.toString).map[Int](_.length) <--- this expression can be pattern matched

  //TODO - a very interesting example
