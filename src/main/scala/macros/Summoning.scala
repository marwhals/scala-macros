  ...
  def describeTypeImplFaulty[A <: Tuple : Type](using Quotes): Expr[String] = {
    Type.of[A] match {
      case '[(_, a, _)] =>
        ...
        // '{ summon[MyTypeClass[a]].message } // this doesn't work --> can't prove this at compile time
  ...
  def describeTypeImpl[A <: Tuple : Type](using q: Quotes): Expr[String] = {
    ...
    Type.of[A] match {
      case '[(_, a, _)] =>
        ...
        val typeClass = maybeTypeClass.getOrElse(report.errorAndAbort(s"[Custom Error!!!!!] missing type class for ${Type.show[a]}"))
