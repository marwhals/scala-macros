...
/**
 * - the point of macros -- transform an AST into another AST -- can be infeasible to do manually.....
 *
 */
...
object TreeMappings {
  ...
  def transformCodeImpl[A: Type](code: Expr[A])(using q: Quotes): Expr[A] = {
    ...
    val treeMap = new TreeMap { // Trees are recursive data structure
      // This will recursively traverse the entire AST of the program.
      // Can pattern match certain pieces of these if we want
      // TreeMaps are necessary for using this in real life
  ...
  inline def flipBooleans[A](inline code: A): A = //Flip all the booleans in the AST.....CNF?
