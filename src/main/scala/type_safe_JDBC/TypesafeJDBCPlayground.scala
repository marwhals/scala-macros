...
object TypesafeJDBCPlayground {
  ...
  def demoRefinedType() = {
    ...
    val rows: Seq[Row] = JDBCCommunication.runQuery(query)
  ...
  def demoRefinedType_v2 = {
    ...
    // can lose the result of the type here causing compilation errors - stay within the macro with computed types
