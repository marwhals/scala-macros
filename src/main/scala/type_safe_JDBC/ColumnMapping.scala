...
object ColumnMapping {
  ...
  type Aux[T <: JDBCType.TL, N <: JDBCNullability.TL, C <: String, R] = ColumnMapping[T, N, C] {type Result = R}
  ...
  given [C <: String, T <: JDBCType.TL, R]
  (using existingMapper: Aux[T, NonNullable, C, R]): ColumnMapping[T, Nullable, C] with {
  ...
  given [C <: String, T <: JDBCType.TL, R]
  (using existingMapper: Aux[T, NonNullable, C, R]): ColumnMapping[JDBCType.TL.Array[T], NonNullable, C] with {
    type Result = List[R]
